...
func TestDelegate_ServicesForSpec(t *testing.T) {
	...
	minRequiredOutgoingConfirmations := uint64(1)
	delegate := directrequest.NewDelegate(broadcaster, headBroadcaster, runner, nil, gethClient, orm.DB, minRequiredOutgoingConfirmations)
...
func TestDelegate_ServicesListenerHandleLog(t *testing.T) {
	...
	minRequiredOutgoingConfirmations := uint64(2)
	delegate := directrequest.NewDelegate(broadcaster, headBroadcaster, runner, orm, gethClient, db, minRequiredOutgoingConfirmations)
	...
	t.Run("Log is an OracleRequest", func(t *testing.T) {
		...
		headBroadcastable.OnNewLongestChain(context.TODO(), models.Head{Number: 10})
	...
	t.Run("Log is not consumed, as it's too young", func(t *testing.T) {
		...
		headBroadcastable.OnNewLongestChain(context.TODO(), models.Head{Number: 0})
		...
		headBroadcastable.OnNewLongestChain(context.TODO(), models.Head{Number: 2})
		runBeganAwaiter.AwaitOrFail(t, 5*time.Second)
	...
	t.Run("Log has wrong jobID", func(t *testing.T) {
		...
		cltest.EventuallyExpectationsMet(t, broadcaster, 3*time.Second, 100*time.Millisecond)
		cltest.EventuallyExpectationsMet(t, runner, 3*time.Second, 100*time.Millisecond)
		cltest.EventuallyExpectationsMet(t, log, 3*time.Second, 100*time.Millisecond)
	...
	t.Run("Log is a CancelOracleRequest with a matching run", func(t *testing.T) {
		...
		headBroadcastable.OnNewLongestChain(context.TODO(), models.Head{Number: 11})
